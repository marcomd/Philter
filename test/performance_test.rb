...
class PerformanceTest < Test::Unit::TestCase
  ...
  test 'benchmark fixnums by array' do
    ...
    time = Benchmark.measure { 600.times { ar_test.philter [1,2] } }
    assert time.real < @required_time, "Philter time #{time.real} too high"
  end

  test 'benchmark fixnums by operator' do
    ar_test = get_big_array_of_fixnums
    time = Benchmark.measure { 290.times { ar_test.philter '< 50' } }
    assert time.real < @required_time, "Philter time #{time.real} too high"
  end

  test 'benchmark strings by reg exp' do
    ar_test = get_big_array_of_fixnums 200
    time = Benchmark.measure { 500.times { ar_test.philter /a/i } }
    assert time.real < @required_time, "Philter time #{time.real} too high"
  end

  test 'benchmark hashes by id' do
    ar_test = get_array_with_hashes
    time = Benchmark.measure { 10_000.times { ar_test.philter id: 1 } }
    assert time.real < @required_time, "Philter time #{time.real} too high"
  end

  test 'benchmark hashes by regexp' do
    ar_test = get_array_with_hashes
    time = Benchmark.measure { 10_000.times { ar_test.philter email: /\A.+gmail/ } }
    assert time.real < @required_time, "Philter time #{time.real} too high"
  end

  test 'benchmark hashes by array of id' do
    ar_test = get_array_with_hashes
    time = Benchmark.measure { 4_000.times { ar_test.philter id: [1, 2, 3] } }
    assert time.real < @required_time, "Philter time #{time.real} too high"
  end
